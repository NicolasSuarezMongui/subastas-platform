<template>
  ...
  <div v-if="productosStore.loading" class="flex flex-col items-center justify-center py-28 gap-4">
    ...
    <p class="text-sm text-gray-600">Cargando subasta...</p>
  </div>
  ...
  <div v-else-if="producto" class="animate-fade-in-up">
    ...
    <RouterLink
      to="/"
      class="inline-flex items-center gap-1.5 text-sm text-gray-600 hover:text-indigo-400 transition-colors mb-6 group"
    >
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 space-y-5">
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
          ...
          <div class="p-6">
            ...
            <p class="text-gray-500 leading-relaxed mb-6 text-sm">{{ producto.descripcion }}</p>
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
              ...
              <div class="bg-gray-800/50 border border-gray-700/40 rounded-xl p-4">
                ...
                <p class="text-3xl font-black text-gray-600 leading-none">
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
          ...
          <div v-else class="divide-y divide-gray-800/60">
            <div
              ...
              :key="puja.id"
              class="flex items-center justify-between px-6 py-3.5 transition-colors"
              :class="index === 0 ? 'bg-indigo-950/20' : 'hover:bg-gray-800/20'"
            >
              <div class="flex items-center gap-3">
                ...
                <span
                  ...
                  :class="{
                    'bg-amber-400 text-gray-950': index === 0,
                    'bg-gray-700/60 text-gray-500': index > 0
                  }"
                >
                  {{ index === 0 ? '👑' : index + 1 }}
                </span>
                <div>
                  <p class="text-sm font-semibold" :class="index === 0 ? 'text-white' : 'text-gray-400'">
                    {{ puja.username }}
                  </p>
                  <p class="text-xs text-gray-600">
                    {{ new Date(puja.createdAt).toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' }) }}
                  </p>
                </div>
              </div>

              <div class="text-right">
                <p class="font-bold tabular-nums" :class="index === 0 ? 'text-amber-400' : 'text-gray-500'">
      ...
      <div class="lg:col-span-1">
        <div class="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden sticky top-24">
          ...
          <div class="p-6">
            <h2 class="font-bold text-white mb-0.5">Realizar puja</h2>
            <p class="text-xs text-gray-600 mb-6">
              Mínimo:
              <span class="text-amber-400 font-semibold">${{ producto.precioActual.toLocaleString() }}</span>
            </p>
            ...
            <div class="space-y-4">
              ...
              <!-- Usuario ID -->
              <div>
                <label class="block text-xs font-medium text-gray-400 mb-1.5">Tu ID de usuario</label>
                <div class="relative">
                  ...
                  <input
                    ...
                    type="number"
                    placeholder="Ej: 1"
                    class="w-full bg-gray-800 border border-gray-700 rounded-xl pl-10 pr-4 py-3 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/20 transition-all"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div class="relative">
                  <span class="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500 font-semibold text-sm">$</span>
                  <input
                    v-model.number="form.monto"
                    type="number"
                    placeholder="0"
                    class="w-full bg-gray-800 border border-gray-700 rounded-xl pl-9 pr-4 py-3 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500/20 transition-all"
                  />
                </div>
                <!-- Minimum hint -->
                <p
                  v-if="form.monto && form.monto <= producto.precioActual"
                  class="text-xs text-amber-500/80 mt-1.5"
                >
                  La puja debe superar ${{ producto.precioActual.toLocaleString() }}
                </p>
              </div>
              ...
              <button
                @click="pujar"
                :disabled="!form.usuarioId || !form.monto || form.monto <= producto.precioActual || pujasStore.loading"
                class="w-full bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 disabled:from-gray-700 disabled:to-gray-700 disabled:text-gray-500 disabled:cursor-not-allowed text-white font-bold py-3.5 rounded-xl transition-all duration-200 shadow-lg shadow-amber-950/40 hover:shadow-amber-900/40"
              >
